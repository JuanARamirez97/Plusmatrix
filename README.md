...
`plusmatrix` simulates the iconic falling characters from The Matrix, featuring dynamic terminal resizing, customizable trail colors, and a clean, low-latency rendering loop using the `ncurses` library.
...
* **Modern C++:** Built using C++20 standard and configured with a modern CMake pipeline.
...
```bash
git clone https://github.com/JuanARamirez97/Plusmatrix.git
cd Plusmatrix
```
...
You can customize the color of the digital rain using the `-c` flag.
```bash
plusmatrix -c <color>
```
